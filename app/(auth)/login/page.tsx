...
export default function LoginPage() {
  ...
  useEffect(() => {
    const checkSupabaseConfig = async () => {
      try {
        // Try to get session to verify Supabase is configured
        const { error: sessionError } = await supabase.auth.getSession()
        if (sessionError && sessionError.message.includes('Invalid')) {
          setError('⚠️ Supabase not configured properly. Check your .env.local file.')
        }
      } catch {
        setError('⚠️ Cannot connect to Supabase. Please add your credentials to .env.local file.')
      }
    }
    checkSupabaseConfig()
  }, [supabase])
