...
import { RECOMMENDATIONS_PROMPT } from '@/lib/ai-prompts'
import {
  ...
  try {
    ...
    if (cachedInsight && !forceRefresh) {
      ...
      if (expiresAt > now && cachedInsight.data_hash === dataHash) {
        return NextResponse.json({
          recommendations: cachedInsight.content as Recommendations,
          cached: true,
    ...
    const { data: recommendations, tokensUsed } = await generateInsight<Recommendations>(
      RECOMMENDATIONS_PROMPT,
      prompt
    )

    // Calculate expiry
    const expiresAt = new Date(now.getTime() + INSIGHT_EXPIRY.recommendations)
